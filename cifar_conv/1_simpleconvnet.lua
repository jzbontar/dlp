...
model:add(nn.SpatialConvolutionRing(1,hiddens[1],5,5))
model:add(nn.SpatialMaxPooling(2,2))
model:add(nn.Threshold(0,0))
-- todo contrastivenormalization
model:add(nn.SpatialZeroPadding(2,2,2,2))
model:add(nn.SpatialConvolutionBatch(hiddens[1],hiddens[2],5,5))
model:add(nn.SpatialMaxPooling(2,2))
model:add(nn.Threshold(0,0))
-- todo contrastivenormalization
model:add(nn.SpatialZeroPadding(2,2,2,2))
model:add(nn.SpatialConvolutionBatch(hiddens[2],hiddens[3],5,5))
model:add(nn.SpatialMaxPooling(2,2))
model:add(nn.Threshold(0,0))
...
for epoch = 1,Nepoch do
    ...
    prof.toc('evaluate')
    if Ete[#Ete] >= Ete[#Ete-1] and Ete[#Ete] >= Ete[#Ete-2] then break end
end
prof.toc('train')

prof.dump()
torch.save('conv.net', model)
